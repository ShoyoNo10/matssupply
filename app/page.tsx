...
export default async function Home() {
  ...
  const categories = JSON.parse(
    JSON.stringify(await CategoryModel.find().sort({ order: 1 }).lean()),
  ) as Category[];

  const subs = JSON.parse(
    JSON.stringify(await SubCategoryModel.find().sort({ order: 1 }).lean()),
  );
  ...
  return (
    <main>
      ...
      <HomeCategoryMenu categories={categories} subs={subs} />
